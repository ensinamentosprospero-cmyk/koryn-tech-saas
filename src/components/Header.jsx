...
export default function Header({ searchQuery = '', onSearchChange, onSearchSubmit }) {
  const { store, sections, messages, copy, whatsAppLink, trackEvent } = useSiteConfig();
  const { openLogin, isAdminAuthenticated, isUserAuthenticated } = useAuth();
  const { cartCount } = useUserData();
  const [panelOpen, setPanelOpen] = useState(false);
  const [scrolled, setScrolled] = useState(false);

  const visibleNavLinks = (copy?.navLinks || []).filter((link) => sections[link.section]);
    ...
    <>
      ...
      >
        <div className="mx-auto flex h-[4.25rem] max-w-7xl items-center gap-3 px-4 sm:gap-4 sm:px-6 lg:px-8">
          ...
          >
            <div className="relative flex h-9 w-9 shrink-0 items-center justify-center overflow-hidden rounded-xl bg-gradient-to-br from-brand-700 via-brand-600 to-brand-800 shadow-glow-brand sm:h-10 sm:w-10">
              {store.logoUrl ? (
